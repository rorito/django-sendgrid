...
from sendgrid.constants import BATCHED_EVENT_SEPARATOR, EVENT_TYPES_EXTRA_FIELDS_MAP, EVENT_MODEL_NAMES, \
    NEWSLETTER_UNIQUE_IDENTIFIER, UNIQUE_ARGS_STORED_FOR_NEWSLETTER_EVENTS
from sendgrid.models import EmailMessage, Event, ClickEvent, DeferredEvent, DroppedEvent, DeliverredEvent, BounceEvent, \
    EventType, Category, Argument, UniqueArgument
...
def build_event(event, email_message, event_types):
    ...
    if timestamp:
        ...
        existingEvents = Event.objects.filter(email_message=email_message, event_type=eventType,
...
def create_event_from_sendgrid_params(params, create=True):
    ...
    if timestamp:
        ...
        # this should be done at the db level but since it was added later it would have needed a data migration that either deleted or updated duplicate events
        # this also might need a combined index, but django orm doesn't have this feature yet: https://code.djangoproject.com/ticket/5805
        existingEvents = Event.objects.filter(email_message=emailMessage, event_type=event_type,
