...
SENDGRID_EMAIL_TRACKING_COMPONENTS = getattr(settings, "SENDGRID_USER_MIXIN_ENABLED",
                                             DEFAULT_SENDGRID_EMAIL_TRACKING_COMPONENTS)
...
class EmailMessage(models.Model):
    ...
    category = models.CharField(max_length=EMAIL_MESSAGE_CATEGORY_MAX_LENGTH, blank=True, null=True,
                                help_text="Primary SendGrid category")
    ...
    def from_event(self, event_dict):
        ...
        try:
            emailMessage = UniqueArgument.objects.get(data=newsletter_id, argument__key=NEWSLETTER_UNIQUE_IDENTIFIER,
            ...
            if event_dict.get("message_id", None):
                logger.debug("DEBUG-EVENT: Trying To Create Email From Event with message_id {0}".format(
                    event_dict.get("message_id", None)))
                existingEmail = EmailMessage.objects.filter(message_id=event_dict.get("message_id", None))
                if len(existingEmail) > 0:
                    logger.debug("DEBUG-EVENT: Found an existing email with message_id {0}".format(
                        event_dict.get("message_id", None)))
